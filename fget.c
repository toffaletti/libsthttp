...
struct http_stream *http_stream_create(void) {
...
int http_stream_request(struct http_stream *s, uri *u) {
  ...
  st_write(s->nfd, req_data->str, req_data->len, ST_UTIME_NO_TIMEOUT);
  ...
  if (httpclient_parser_is_finished(&s->clp) && !httpclient_parser_has_error(&s->clp)) {
    ...
    s->transfer_encoding = http_response_header_getstr(&s->resp, "Transfer-Encoding");
...
static void *do_get(void *arg) {
  ...
  char *uri_s = (char *)arg;
  struct http_stream *s = http_stream_create();
